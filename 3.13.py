...
lambd = [0.5, 0.8, 1.3]
s = [0.89511223, 1.20110178, 1.43964151]
kap = [0.2, 0.70137457, 1]
...
for i in range(3):
    ...
    for j in range(3):
        T1Scenarios[i] = utslappscen(UtslappScenarios[i], lambd[j], s[j], kap[j])
        axes[i].plot(year, T1Scenarios[i,:], label='lambda = {}'.format(lambd[j]))

    tempaverage = np.mean(utslappscen(UtslappScenarios[i], 0.8, s[1], kap[1])[186:216])
    axes[i].plot(years, np.add(temperature_anomalies, tempaverage), label='Observed', color='red')
    axes[i].set_xlim([1880, 2200])
    axes[i].set_ylim([-1.2, 5.6])
    axes[i].autoscale_view()

    # axes[i].ylim(-1.2, 3.6)
    axes[i].hlines(0, 1765, 2200, colors='black', linestyles='dashed')
    axes[i].legend()
    axes[i].set_title('Scenario {}'.format(i+1))
    axes[i].set_xlabel('Year')
    axes[i].set_ylabel('Temperature Anomaly (°C)')



# plt.figure()

fig.suptitle('Global Temperature Anomalies Over Time (NASA GISS)')
